<template>
	<div class="enterprise-to-workmachine-tree">
		<el-tree :data="getEnterpriseToWorkmachineTreeData" :props="defaultProps" highlight-current
						 :default-expand-all="expandAll"
						 @node-click="handleNodeClick" v-on="$listeners"
						 v-loading="this.$asyncComputed.getEnterpriseToWorkmachineTreeData.updating"></el-tree>
	</div>
</template>

<script>
	export default {
		name: "enterpriseToWorkmachineTree",
		props:{
			machineType: {
				type: String,
				default: ''
			},
			workCellType: {
				type: String,
				default: ''
			},
			machineNameFilter:{
				type: String,
				default: ''
			},
			expandAll: {
				type: Boolean,
				default: false,
			},
		},
		data(){
			return {
				defaultProps: {
					children: 'children',
					label: 'name'
				},
				currentNode:{},
			}
		},
		asyncComputed:{
			async getEnterpriseToWorkmachineTreeData() {
				let fd = new FormData();
				fd.set('flag', 'getMachineTreeList');
				fd.set('areaId','[]');
				fd.set('machineTypeId',this.machineType);
				fd.set('workCellTypeId',this.workCellType);
				fd.set('hasTube',false);
				fd.set('nameContain',this.machineNameFilter);
				let list = (await this.$axios.post('/mes/Service/FactoryConfigService.ashx', fd)).data;
				let tree=[];
				let enterprises= list.filter((item)=>{ if (item.parentId==null){return item;}});
				if (enterprises){
					for (let i=0;i<enterprises.length;i++){
						let sites= list.filter((item)=>{ if (item.parentId==enterprises[i].id){return item;}});
						if (sites){
							for (let j=0;j<sites.length;j++){
								let areas= list.filter((item)=>{ if (item.parentId==sites[j].id){return item;}});
								if (areas){
									for (let k=0;k<areas.length;k++){
										let cells= list.filter((item)=>{ if (item.parentId==areas[k].id){return item;}});
										if (cells){
											for (let l=0;l<cells.length;l++){
												let machines= list.filter((item)=>{ if (item.parentId==cells[l].id){return item;}});
												if (machines){
													cells[l].children=machines;
												}
											}
											areas[k].children=cells;
										}
									}
									sites[j].children=areas;
								}
							}
							enterprises[i].children=sites;
						}
						tree.push(enterprises[i]);
					}
				}
				return tree;
			}
		},
		methods:{
			handleNodeClick(data){
				this.currentNode=data;
				this.$emit('nodeClick',data);
			},
		}
	}
</script>

<style scoped>

</style>